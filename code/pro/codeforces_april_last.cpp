	...
	while(T--){
		...
		reps(i,1,n){
			...
			for(pll fe : vec)
				...
				{
					cout <<1<< " " <<  i  << " " <<  fe.X << " " << fe.Y<< endl;
					que.push_back(fe);
				}
			while(que.size() && que.front().X+t<i) que.pop_front();
			if(que.size()) dp[i]=que.front().Y+a[i];
			while(que.size() && que.back().Y>=dp[i]) que.pop_back();
			que.eb(i,dp[i]);
			out(i,que,1);
			vec.clear();
